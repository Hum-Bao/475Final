...
void Transaction::GetTransactionByCustomer(std::string customer_email) {
    try {
        SACommand select(
            &con, "SELECT * FROM Transactions WHERE customer_email = :1");
        ...
        while (select.FetchNext()) {
            std::cout << "Transaction ID: "
                      << select.Field("transaction_id").asLong() << "\n";
            std::cout
                << "Customer Email: "
                << select.Field("customer_email").asString().GetMultiByteChars()
                << "\n";
            std::cout
                << "Facility Name: "
                << select.Field("facility_name").asString().GetMultiByteChars()
                << "\n";
            std::cout
                << "Goods Name: "
                << select.Field("goods_name").asString().GetMultiByteChars()
                << "\n";
            std::cout << "-------------------------------\n";
        }
    } catch (SAException& ex) {
        std::cerr << "Error fetching transactions: "
                  << ex.ErrText().GetMultiByteChars() << std::endl;
    }
}
...
void Transaction::GetTransactionByFacility(std::string facility_name) {
    try {
        SACommand select(&con,
                         "SELECT * FROM Transactions WHERE facility_name = :1");
        ...
        std::cerr << "Error fetching transactions by facility: "
                  << ex.ErrText().GetMultiByteChars() << std::endl;
    }
}
...
void Transaction::GetTransactionByDateRange(std::string start_date,
    ...
    try {
        ...
        std::cerr << "Error fetching transactions by date range: "
                  << ex.ErrText().GetMultiByteChars() << std::endl;
    }
}*/